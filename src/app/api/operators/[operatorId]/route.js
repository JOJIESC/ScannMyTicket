import { NextResponse } from "next/server";
import { conn } from "@/libs/mysql";
import { cookies } from "next/headers";
import { jwtVerify } from "jose";

async function canManageByOperator(operatorId) {
  const token = cookies().get("ScannToken")?.value;
  if (!token) return false;
  const sec = process.env.JWT_SECRET;
  if (!sec) return false;

  const { payload } = await jwtVerify(token, new TextEncoder().encode(sec));
  const me = payload?.user;
  if (!me) return false;

  // Buscar dueño del evento al que pertenece el operador
  const rows = await conn.query(
    `SELECT e.user_id
       FROM operators o
       JOIN events e ON e.id = o.event_id
      WHERE o.id = ?`,
    [operatorId]
  );
  if (!Array.isArray(rows) || rows.length === 0) return false;

  const isOwner = String(rows[0].user_id) === String(me.id);
  const isAdmin = String((me.role || "")).toUpperCase() === "ADMIN";
  return isOwner || isAdmin;
}

export async function DELETE(_req, { params }) {
  try {
    const operatorId = params.operatorId;
    if (!operatorId) return NextResponse.json({ message: "Falta operatorId" }, { status: 400 });

    const allowed = await canManageByOperator(operatorId);
    if (!allowed) return NextResponse.json({ message: "Forbidden" }, { status: 403 });

    const res = await conn.query("DELETE FROM operators WHERE id = ?", [operatorId]);
    if (res.affectedRows === 0) {
      return NextResponse.json({ message: "Operador no encontrado" }, { status: 404 });
    }
    return NextResponse.json({ message: "Operador eliminado" }, { status: 200 });
  } catch (e) {
    console.error("operators delete error:", e);
    return NextResponse.json({ message: "Error eliminando operador" }, { status: 500 });
  }
}
